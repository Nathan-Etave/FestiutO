...
class Utilisateur:
    ...
    class Insert:
        ...
        def insert_user(mail,prenom,nom,mdp):
            ...
            try:
                session = Session()
                user = UTILISATEUR(idU=Utilisateur.Get.get_last_idU() + 1, nomU=nom, prenomU=prenom, mailU=mail, mdpU=mdp, idR=3)
                session.add(user)
                session.commit()
            except:
                raise
...
class Concert:
    ...
    class Insert:
        ...
        def insert_concert(idG,idL,dateDebC,dateFinC,dureeMontageC,dureeDemontageC,estGratuit):
            try:
                session = Session()
                concert = CONCERT(idC=Concert.Get.get_last_idC() + 1, idF=1, idG=idG, idL=idL, dateDebC=dateDebC, dateFinC=dateFinC, dureeMontageC=dureeMontageC, dureeDemontageC=dureeDemontageC, estGratuit=estGratuit)
                session.add(concert)
                session.commit()
            except:
                raise
...
class Groupe:
    ...
    class Get:
        ...
        def get_groupe_related(idG):
            try:
                style_musical = Groupe.Get.get_groupe_with_idG(idG).STYLEMUSICAL.idS
                session = Session()
    ...
    class Insert:
        ...
        def insert_groupe(nomG,idS,descG):
            try:
                session = Session()
                groupe = GROUPE(idG=Groupe.Get.get_last_idG() + 1, idS=idS, nomG=nomG, descriptionG=descG)
                session.add(groupe)
                session.commit()
            except:
                raise
    ...
    class Update:
        ...
        def update_groupe(idG,nomG,idS,descG, images):
            try:
                ...
                liste_images = [] 
                for image in images:
                    photo = PHOTO(idP=Photo.Get.get_last_idP() + 1, img=base64.b64encode(image.read()))
...
class Hebergement:
    ...
    class Insert:
        ...
        def insert_hebergement(nomH,adresseH,nbPlacesH):
            try:
                session = Session()
                hebergement = HEBERGEMENT(idH=Hebergement.Get.get_last_idH() + 1, nomH=nomH, adresseH=adresseH, nbPlacesH=nbPlacesH)
...
class Billet:
    ...
    class Insert:
        ...
        def insert_billet(idT,idU,dateDebB, dateFinB, quantite):
            ...
            try:
                ...
                for i in range(quantite):
                    billet = BILLET(idB=Billet.Get.get_last_idB() + i + 1, idT=idT, idU=idU, idF=1, dateDebB=dateDebB, dateFinB=dateFinB)
                    session.add(billet)
                session.commit()
            except:
                raise
        ...
        def add_billet_to_panier(idU, idT, dateDebB, dateFinB):
            try:
                session = Session()
                billet = BILLET(idB=Billet.Get.get_last_idB() + 1, idT=idT, idU=idU, idF=1, dateDebB=dateDebB, dateFinB=dateFinB)
                session.add(billet)
                session.commit()
...
class Artiste:
    ...
    class Insert:
        ...
        def insert_artiste(nomA,prenomA,idG):
            try:
                session = Session()
                artiste = ARTISTE(idA=Artiste.Get.get_last_idA() + 1, nomA=nomA, prenomA=prenomA, idP=1, idG=idG)
                session.add(artiste)
                session.commit()
            except:
                raise
...
class Activites:
    ...
    class Insert:
        ...
        def insert_activite(idG,nomA,descA,idL,dateDebA,dateFinA,estPublique):
            try:
                session = Session()
                activite = ACTIVITE_ANNEXE(idAct=Activites.Get.get_last_idAct() + 1, nomAct=nomA, descriptionAct=descA, dateDebAct=dateDebA, dateFinAct=dateFinA, idL=idL, idF=1, idG=idG, estPublique=estPublique)
    ...
    class Get:
        ...
        def get_favoris(idU):
            try:
                ...
                favoris = []
                for fav in all_fav:
                    favoris.append(Groupe.Get.get_groupe_with_idG(fav.idG))
                return favoris
            except:
                raise
            finally:
